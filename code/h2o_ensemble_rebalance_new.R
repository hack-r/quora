...
fit <- h2o.ensemble(x              = setdiff(colnames(x_train2), "y"),
                    y              = "y",
...
# More CV folds will take longer to train, but should increase performance
s <- system.time()
  fit <- h2o.ensemble(x              = setdiff(x_names, "y"),
                      y              = "y",
                      training_frame = trainHex,
                      metalearner    ="h2o.randomForest.wrapper",
                      learner        = learner)
...
saveRDS(sample, "big_ens2new.RDS")
fwrite(sample,  "big_ens2new.csv", row.names = F)

saveRDS(base, "big_ens_base2new.RDS")
