...
x_train2$same_word_count <- as.numeric(as.character(x_train2$same_word_count))
x_valid2$same_word_count <- as.numeric(as.character(x_valid2$same_word_count))
x_test2$same_word_count  <- as.numeric(as.character(x_test2$same_word_count))

t1_sparse <- Matrix(as.matrix(x_train2), sparse=TRUE)
s1_sparse <- Matrix(as.matrix(x_test2), sparse=TRUE)
v1_sparse <- Matrix(as.matrix(x_valid2), sparse=TRUE)

labels             <- y_train$V1 #train$is_duplicate[1:400000]
valid.labels       <- y_valid$V1 #train$is_duplicate[400001:nrow(train)]
...
dtrain <- lgb.Dataset(data = as.matrix(x_train2), 
                      label = y_train$V1,
                      free_raw_data = FALSE
...
pred.valid <- predict(bst, as.matrix(v1_sparse))
...
# Save Predictions --------------------------------------------------------
saveRDS(pred.valid, "new_lgb_pred_valid.RDS")
saveRDS(score,      "new_lgb_pred_score_gbdt.RDS")
fwrite(as.data.frame(score), "new_lgb_pred_score_gbdt.csv")
